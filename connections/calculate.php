...
if ($calculate == "premise") {
    ...
    $cloudI = fetchCO2eFromCloudEmissions($region);
...
function calculatePremiseE(
    ...
    if ($power_consumption_known == "false") {
        if ($cpu == 1) {
...
function calculatePremiseI($renewable_percentage, $location)
{
    ...
    if ($renewable_percentage == "NULL") {
        return $emissions_factors;
    } else if ($renewable_percentage >0 && $renewable_percentage <100) {
        return (1 - $renewable_percentage / 100) * $emissions_factors;
    } else if($renewable_percentage  >= 100){
        return 0;
    }
}
